 ...
 */
public class ProcessMultM extends Process{
	
	private Matrix m1,m2;
	private final static int COEF = 20;//coef de l'operation
	 ...
	 * @param m2 deuxieme matrice à multiplier
	 * @param clientname le nom de client demandeur de traitement
	 * @param ps la capacité de serveur
	 */
	public ProcessMultM(ProcessingAppInterface pa, ManagerAppInterface ma, Queue q, Matrix m1, Matrix m2, String clientname, int ps) {
		...
		message = new String("Matrix multiplication, client " + clientname + "\n");
	}
		...
		for(i=0;i<nbl1;i++){
			...
			for(j=0;j<nbc2;j++){
				...
				try {
					...
					M.setValue(i, j, 0); //initialisation
					
					for(k=0; k<nbc1; k++){
						
						M.setValue(i, j, M.getValue(i, j) + m1.getValue(i, k)*m2.getValue(k, j));
					}
				} 
